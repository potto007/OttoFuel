    ...
    internal class Fireplace_Patches
    {
            ...
            private static void Postfix(Fireplace __instance, ZNetView ___m_nview)
            {
               ...
               // OttoFuel.OttoFuelPlugin.OttoFuelLogger.LogInfo(__instance.name);
                ...
                if (Time.time - OttoFuelPlugin.lastFuel < 0.1)
                {
                    ...
                    RefuelTorch(__instance, ___m_nview, OttoFuelPlugin.fuelCount * 33);
                }
                else
                {
        ...
        public static async void RefuelTorch(Fireplace fireplace, ZNetView znview, int delay)
        {
            try
            {
                ...
                if (!fireplace || !znview || !znview.IsValid() || !OttoFuelPlugin.modEnabled.Value)
                    return;
                ...
                List<Container> nearbyContainers = TastyUtils.GetNearbyContainers
                    (fireplace.transform.position, OttoFuelPlugin.fireplaceRange.Value);
